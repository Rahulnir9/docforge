...
export default function App() {
  ...
  function handleFile(f) {
    if (!f) return
    if (!f.name.endsWith(".py") && !f.name.endsWith(".zip")) {
      setError("Only .py or .zip files are supported.")
      return
    }
    setFile(f)
    setError("")
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (res.data.total === 0) {
        setError("No API routes found. Make sure your file uses FastAPI decorators like @app.get()")
        setLoading(false)
        return
      }
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <p style={styles.sub}>Upload a FastAPI .py file or a .zip of your project. Get instant interactive documentation.</p>
        ...
        >
          ...
          <p style={styles.dropSub}>{file ? `${(file.size/1024).toFixed(1)} KB` : "or click to browse — .py or .zip"}</p>
          <input
            type="file" accept=".py,.zip"
